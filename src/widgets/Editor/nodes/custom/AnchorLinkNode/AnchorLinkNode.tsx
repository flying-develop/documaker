...
export class AnchorLinkNode extends DecoratorNode<JSX.Element> {
    __text: string;
    __id: string;
    ...
    static clone(node: AnchorLinkNode): AnchorLinkNode {
        return new AnchorLinkNode(node.__text, node.__id, node.__key);
    }
    ...
    static importJSON(serializedNode: SerializedLinkNode): AnchorLinkNode {
    ...
    updateDOM(prevNode: AnchorLinkNode): boolean {
...
export function $createLinkNode(text = '', id = ''): AnchorLinkNode {
    const linkNode = new AnchorLinkNode(text, id);
    return $applyNodeReplacement(linkNode);
}

export function $isLinkNode(node: LexicalNode | null | undefined): node is AnchorLinkNode {
    return node instanceof AnchorLinkNode;
}
